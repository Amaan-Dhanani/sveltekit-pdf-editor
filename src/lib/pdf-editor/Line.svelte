...
<div
    ...
    role="presentation"
    class="pointer-events-none absolute left-0 top-0 select-none touch-none [-webkit-user-drag:none]"
    style="
        width: {Math.abs(shapeWidth) + strokeWidth + 12}px;
        height: {Math.abs(shapeHeight) + strokeWidth + 12}px;
        transform: translate({x + Math.min(0, shapeWidth) - 6}px, {y +
...
>
    <svg width="100%" height="100%" class="overflow-visible">
        <line
            ...
            class="[pointer-events:stroke] outline-none cursor-pointer"
    ...
    {#if selected}
        <div
            class="resize-handle absolute h-3 w-3 cursor-move rounded-full border-2 border-white bg-blue-500 transition-colors hover:bg-blue-600 pointer-events-auto"
